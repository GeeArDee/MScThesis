\chapter{Facility design}
    ...
    \section{Version 2} \label{sec:design_v2}
        ...
        \subsection{Initial sizing of the double choked LTP thruster}
            ...
            Starting with a cold gas thruster using argon, the speed of sound of argon ($c_0$) is \qty{323}{m/s}. This is at ambient temperature (\qty{300}{K}), as there is no laser energy to heat the gas in this case. With a nozzle, the gas is accelerated to approximately twice this speed. The exit velocity of the gas $v_\mathrm{exit}$, which is the main performance parameter, is therefore \qty{646}{m/s}.
            
            %---BEFORE
            Laser on (hot) operation will now be examined. 
            
            Taking the previous $v_\mathrm{exit}$ and ionizing the whole flow, it is assumed that the thruster's efficiency is doubled. 
            
            This gives a $v_\mathrm{exit}\approx \qty{1300}{m/s}$. 
            
            The nozzle throat size will therefore be found for this $\dot m$, with $p_\mathrm{chamber} = \qty{25}{bar}$. 
            
            $\mathrm{MW_{Ar}} = \qty{40}{g/mol}$ and the speed of sound is $c = \sqrt{\gamma R T}$. 
            
            As the speed of sound will be doubled, the temperature is multiplied by 4.
            % ---AFTER
            As the laser power is fixed, 

            

            To see an effect on thrust that was significant, the mass flow was set such that the thrust would be doubled with mass flow rate kept fixed. Perfect conversion of laser power to gas heating was assumed.
